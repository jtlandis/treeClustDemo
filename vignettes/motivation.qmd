...
se <- SummarizedExperiment(
  assays = list(counts = matrix(rpois(1000000, 10), 10000, 100)),
  rowData = DataFrame(
    feature = sprintf("f%04i", 1:10000),
    length = sample(1e3:1e5, 10000, replace = TRUE)
  ),
  colData = DataFrame(
    sample = sprintf("s%02i", 1:100)
  )
)

.rowtree <- ape::rtree(10000)
...
ggtree(.rowtree, size = 2, branch.length = "none") +
...
```{r}
print(xp)
xp <- xp |> group_by_nodes(rows(parent_nodes(tree))) |>
  summarize(counts = colSums(counts))
print(xp)
xp <- xp |> group_by_nodes(rows(parent_nodes(tree))) |>
  summarize(counts = colSums(counts))
