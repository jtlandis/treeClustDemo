...
pull_tse <- function(tse, which = c("row", "col")) {
  ...
  switch(
    ...
    col = {
      lnks <- colLinks(tse)
      trees <- colTree(tse, whichTree = NULL)
    },
    row = {
      lnks <- rowLinks(tse)
      trees <- rowTree(tse, whichTree = NULL) 
    }
  )
  which_tree <- match(lnks$whichTree, names(trees))
  node_id <- lnks$nodeNum
  new_tree_vctr(
    node = node_id,
  ...
  # tree_labels <- vctrs::vec_chop(lnks$nodeNum, indices = chops$loc)
...
<!-- below is taken from treeclimbR vignette (I think) -->
...
microbenchmark::microbenchmark(
  treeClimbR = aggTSE(x = da_lse, rowLevel = all_node, rowFun = sum),
  plyxp = group_by_nodes(
    xplse, rows(all_nodes(tree))
  ) |>
  summarise(counts = colSums(counts)) 
) |> print()
